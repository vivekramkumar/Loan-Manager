...
const ItemState = props => {
  ...
  const getPendingItems = async () => {
    ...
    try {
      const res = await axios.get(baseURL + '/api/items/pendingLoans');
      dispatch({
        type: GET_ITEMS,
        payload: res.data
      });
  ...
  // Approve Loan
  const ApproveLoan = async (category,itemId) => {
    showLoading();
    try {
      const res = await axios.put(baseURL + `/api/items/approveLoan/${itemId}`);
  ...
  // Reject Loan
  const RejectLoan = async (itemId) => {
    showLoading();
    try {
      const res = await axios.put(baseURL + `/api/items/rejectLoan/${itemId}`);
      //item["itemId"] = res.data.itemId;
      dispatch({
        type: UPDATE_ITEM,
        payload: res.data,
